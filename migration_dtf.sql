...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS dtf_complete BOOLEAN DEFAULT false;

-- ── PASSO 2: ATUALIZAR A FUNÇÃO RPC 'get_orders_with_stages' ──────────────
-- Como a função RPC retorna colunas específicas, ela precisa ser recriada para
-- incluir a nova coluna 'dtf_complete' no retorno.
-- Execute o script abaixo para atualizar a função no Supabase:
